    ...
    while(i<n-1):
        ...
        while(j<=n-1):
            if(matrix[j][i]!=0.0 and  matrix[i][i]!=0.0):
                alpha=truncate(matrix[j][i])/truncate(matrix[i][i])
                k=0
                while(k<n):
                    temp[0][k]=truncate(alpha*(truncate(matrix[i][k])))
                    temp2[0][k]=truncate(alpha*(truncate(uniqueMatrix[i][k])))
                    k=k+1
                c=0
                while(c<n):
                    matrix[j][c]=truncate(matrix[j][c])-truncate(temp[0][c])
                    #khate baadi baraye be dast avordane inverse e
                    uniqueMatrix[j][c]=truncate(uniqueMatrix[j][c])-truncate(temp2[0][c])
                    c=c+1
            elif(matrix[i][i]==0):
